...
class TSV_UL_groups(bpy.types.UIList):
    ...
    def draw_item(self, context, layout, data, item_8170E, icon, active_data, active_propname, index_8170E):
        ...
        row_FC6FF.prop(bpy.context.scene.sna_tsv_emitter.modifiers['vegetation'].node_group.nodes[str(index_8170E) + '_biome'].inputs[7], 'default_value', text='', icon_value=(253 if bpy.context.scene.sna_tsv_emitter.modifiers['vegetation'].node_group.nodes[str(index_8170E) + '_biome'].inputs[7].default_value else 254), emboss=False)
        row_FC6FF.prop(bpy.context.scene.sna_tsv_emitter.modifiers['vegetation'].node_group.nodes[str(index_8170E) + '_biome'].inputs[8], 'default_value', text='', icon_value=(257 if bpy.context.scene.sna_tsv_emitter.modifiers['vegetation'].node_group.nodes[str(index_8170E) + '_biome'].inputs[8].default_value else 258), emboss=False)
...
class TSV_PT_groups(TSV_PT_panel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row_6C0C6.template_list('TSV_UL_groups', coll_id, bpy.context.scene.sna_tsv_emitter, 'sna_tsv_group_layers', bpy.context.scene.sna_tsv_emitter, 'sna_tsv_group_layer_index', rows=0)
