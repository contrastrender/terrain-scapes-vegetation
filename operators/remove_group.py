...
def sna_remove_biome_06ED5(Index):
    if property_exists("bpy.context.scene.tsv_emitter.tsv_groups[bpy.context.scene.tsv_emitter.tsv_group_index]", globals(), locals()):
        bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes.remove(node=bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes[str(Index) + '_biome'], )
        for i_40409 in range(len(bpy.context.scene.tsv_emitter.tsv_groups[bpy.context.scene.tsv_emitter.tsv_group_index].layers)):
            sna_remove_layer_3CD98(0)
        for i_A1EA6 in range(int(len(bpy.context.scene.tsv_emitter.tsv_groups[bpy.context.scene.tsv_emitter.tsv_group_index].density_masks) + 2.0)):
            bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes.remove(node=bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes[str(Index) + ',' + str(int(i_A1EA6 - 1.0)) + '_density'], )
        for i_64844 in range(int(int(len(bpy.context.scene.tsv_emitter.tsv_groups) - Index) - 1.0)):
            bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes[str(int(int(i_64844 + Index) + 1.0)) + '_biome'].name = str(int(i_64844 + Index)) + '_biome'
            for i_8C52A in range(len(bpy.context.scene.tsv_emitter.tsv_groups[int(int(i_64844 + Index) + 1.0)].layers)):
                bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes[str(int(int(i_64844 + Index) + 1.0)) + ',' + str(i_8C52A) + '_layer'].name = str(int(i_64844 + Index)) + ',' + str(i_8C52A) + '_layer'
            for i_040CE in range(int(len(bpy.context.scene.tsv_emitter.tsv_groups[int(int(i_64844 + Index) + 1.0)].density_masks) + 2.0)):
                bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes[str(int(int(i_64844 + Index) + 1.0)) + ',' + str(int(i_040CE - 1.0)) + '_density'].name = str(int(i_64844 + Index)) + ',' + str(int(i_040CE - 1.0)) + '_density'
        ...
        link_C9799 = bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.links.new(input=bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes[str(bpy.context.scene.tsv_emitter.tsv_group_index) + '_biome'].inputs[0], output=bpy.context.scene.tsv_emitter.modifiers['vegetation'].node_group.nodes[str(int(bpy.context.scene.tsv_emitter.tsv_group_index - 1.0)) + '_biome'].outputs[0], )
        if len(bpy.context.scene.tsv_emitter.tsv_groups) > bpy.context.scene.tsv_emitter.tsv_group_index:
            bpy.context.scene.tsv_emitter.tsv_groups.remove(bpy.context.scene.tsv_emitter.tsv_group_index)
        if (len(bpy.context.scene.tsv_emitter.tsv_groups) == bpy.context.scene.tsv_emitter.tsv_group_index):
